...
let%test_unit "mul" =
  ...
  Test.run_exn
    ~f:(fun m ->
      ...
      let actual = V.Blocked_matrix.(to_matrix (m * m)) in
      [%test_result: Bit_matrix.t] ~expect actual)
    (module V.Blocked_matrix)

let%expect_test "to_matrix" =
  let module VM = V.Blocked_matrix in
  let zero = VM.{ m_buf = "\x00\x00\x00\x00\x00\x00\x00\x00"; m_dim = 1 } in
  Fmt.pr "%a\n" Bit_matrix.pp (VM.to_matrix zero);
  let id =
    VM.{ m_buf = String.rev "\x80\x40\x20\x10\x08\x04\x02\x01"; m_dim = 1 }
  in
  Fmt.pr "%a\n" Bit_matrix.pp (VM.to_matrix id);
  Fmt.pr "%a\n" Bit_matrix.pp VM.(to_matrix (id * id))
